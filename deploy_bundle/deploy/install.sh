...
setup_autostart() {
    ...
    local kiosk_script="$DEPLOY_DIR/open_login_kiosk.sh"
...
EOF
    ...
    if [ -n "$kiosk_script" ]; then
        autostart_exec="/bin/bash $kiosk_script $login_url"
    else
        autostart_exec="xdg-open $login_url"
    fi
...
EOF
    ...
    local desktop_exec
    if [ -n "$kiosk_script" ]; then
        desktop_exec="/bin/bash -c \"\"$run_script\" & sleep 3 && /bin/bash $kiosk_script $login_url\""
    else
        desktop_exec="/bin/bash -c \"\"$run_script\" & sleep 3 && xdg-open $login_url\""
    fi
