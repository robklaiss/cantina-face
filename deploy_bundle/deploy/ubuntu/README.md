...
### Instalación desde USB sellado (OS-FLEX)

```bash
sudo bash /media/$USER/OS-FLEX/deploy/ubuntu/install.sh
```

Antes de llevar el USB al equipo objetivo, generá/actualizá su contenido con:

```bash
bash deploy/usb/make_usb_offline.sh
```

Ese script construye la carpeta `deploy/usb/OS-FLEX/` con todo lo necesario para operar offline.
...
### Layouts de USB soportados (OS-FLEX)

El instalador detecta automáticamente cualquiera de estos escenarios cuando se ejecuta desde `/media/$USER/OS-FLEX`:

1. **Layout A — Sellado completo**
   - `project/` con el código ya descomprimido.
   - `models/arcface_r50.onnx` en la raíz.
   - `deploy/ubuntu/` + `deploy/project.zip` (para referencia/actualización).
2. **Layout B — Histórico mínimo**
   - Sólo `deploy/project.zip` y `deploy/ubuntu/`.
   - El instalador crea `project/` automáticamente extrayendo el zip y limpia basura de macOS.

En ambos casos, `arcface_r50.onnx` se toma en este orden: `models/` → `project/models/` → dentro del zip. Si no aparece, el instalador aborta con instrucciones claras.
