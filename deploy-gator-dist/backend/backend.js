...
const API = {
    ...
    userDetail: (id) => `/backend.php?action=user&id=${id}`,
...
async function loadStudents() {
    const students = await studentsDirectory?.load();
...
async function openStudentLinksModal(studentId) {
    ...
    const cachedStudent = studentsCache.find((student) => String(student.id) === String(studentId));
    ...
    try {
        ...
        const student = payload?.student || cachedStudent || {};
...
async function openUserModal(userId = null) {
    ...
    if (userId && (!user || !user.hashed_password) && supportsUserDetailEndpoint) {
        try {
            user = await apiFetch(API.userDetail(userId));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (user) {
                ...
                await apiFetch(API.userDetail(user.id), {
                    method: 'PUT',
