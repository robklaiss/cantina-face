...
function renderStudents(students = []) {
    ...
    students.forEach((student) => {
        ...
        row.innerHTML = `
            ...
            <td class="actions"><button class="ghost-btn" data-student-id="${student.id}">Copiar ID</button></td>
        `;
        studentsTableBody.appendChild(row);
    });

    studentsTableBody.querySelectorAll('button[data-student-id]').forEach((btn) => {
...
function renderCajaStatus(status) {
    ...
    if (updateAvailable) {
        html += `
            <p class="warning"><strong>⚠️ Actualización disponible:</strong> ${remoteVersion}</p>
        `;
    } else {
        html += `
            <p class="success"><strong>✓ Sistema actualizado</strong></p>
        `;
    }
