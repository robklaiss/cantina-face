...
import numpy as np
import hnswlib
from fastapi import FastAPI, UploadFile, File, Form, HTTPException, Query, Depends, status, Request
...
from config import (
    ...
    EF_CONSTRUCTION,
    M_INDEX,
    RECOGNITION_MIN_INTERVAL_MS,
    NO_FACE_BACKOFF_MS,
    MAX_NO_FACE_BACKOFF_MS,
...
EMBED_COOLDOWN_MS = int(1000 / FACE_MAX_EMB_PER_SEC) if FACE_MAX_EMB_PER_SEC > 0 else 0
FACE_CACHE_WINDOW_MS = max(0, FACE_CACHE_MS)
FACE_CACHE_IOU_THRESHOLD = max(0.0, min(1.0, FACE_CACHE_IOU))

# Burst enrollment dedup: {"name|grade" -> {"student_id": ..., "ts": ...}}
_burst_dedup_cache: Dict[str, Dict[str, Any]] = {}
BURST_DEDUP_WINDOW_S = 30  # seconds


def load_env_claves() -> None:
    env_file = Path(__file__).resolve().parent / ".env-claves"
    ...
    try:
        ...
        print(f"[load_env_claves] warning: {exc}")
...
logger = logging.getLogger("cantina")
perf_logger = logging.getLogger("cantina.perf")
if os.getenv("PERF_LOG", "0") == "1":
...
def _log_stage(stage: str, start_ts: float, **extra: Any) -> None:
    ...
    perf_logger.info(
        "stage=%s duration_ms=%.2f extra=%s",
        stage,
        duration_ms,
        extra or {},
    )
...
def _calc_latency_stats(entries: List[Dict[str, Any]], key: str = "duration_ms") -> Dict[str, float]:
    durations = [float(entry.get(key, 0.0)) for entry in entries if entry.get(key) is not None]
    if not durations:
...
EMBEDDING_DIM = face_engine.get_embedding_dim()
...
async def timing_middleware(request: Request, call_next):
    ...
    try:
        ...
        perf_logger.exception(
            "request_failed path=%s method=%s duration_ms=%.2f error=%s",
            request.url.path,
            request.method,
            duration_ms,
            exc,
...
def ensure_db_indexes() -> None:
    statements = [
        ...
        "CREATE INDEX IF NOT EXISTS idx_transactions_pos ON transactions (point_of_sale_id)",
        "CREATE INDEX IF NOT EXISTS idx_productstock_product_pos ON productstock (product_id, point_of_sale_id)",
        "CREATE INDEX IF NOT EXISTS idx_balanceadjustment_student ON balanceadjustment (student_id)",
        "CREATE INDEX IF NOT EXISTS idx_balanceadjustment_created_at ON balanceadjustment (created_at)",
        "CREATE INDEX IF NOT EXISTS idx_scheduledorder_student_status ON scheduledorder (student_id, status)",
        "CREATE INDEX IF NOT EXISTS idx_student_point_of_sale ON student (point_of_sale_id)",
    ]
...
def _ensure_default_admin_with_credentials(email: str, password: str) -> None:
    try:
        with Session(engine) as session:
            ...
            session.commit()
            print(f"[ensure_default_admin] {email} creado")
    except Exception as exc:
...
def initialize_legacy_data():
    ...
    ensure_column_exists("user", "plain_password", "TEXT")
    default_email = os.getenv("ADMIN_EMAIL", "admin@siloe.com.py")
    default_password = os.getenv("ADMIN_PASSWORD", "admin321")
    _ensure_default_admin_with_credentials(default_email, default_password)
...
def ensure_default_admin():
    admin_email = os.getenv("ADMIN_EMAIL", "admin@siloe.com.py")
    admin_password = os.getenv("ADMIN_PASSWORD", "admin321")
    if not admin_email or not admin_password:
        return

    _ensure_default_admin_with_credentials(admin_email, admin_password)
...
current_no_face_backoff_ms = 0

# Face perf throttling state
face_perf_window = deque(maxlen=512)
request_metrics_window = int(os.getenv("REQUEST_METRICS_MAX", "400"))
request_metrics: deque[Dict[str, Any]] = deque(maxlen=request_metrics_window)
last_embedding_ts_ms = 0
last_embedding_result: Dict[str, Any] | None = None
last_bbox: Optional[Tuple[int, int, int, int]] = None
last_embedding_vector: Optional[np.ndarray] = None

# In-memory products cache (avoids DB hit on every /api/products call)
_products_cache: Optional[List[Dict[str, Any]]] = None
_products_cache_ts: float = 0.0
_PRODUCTS_CACHE_TTL_S: float = float(os.getenv("PRODUCTS_CACHE_TTL", "5"))


def _invalidate_products_cache() -> None:
    global _products_cache, _products_cache_ts
    _products_cache = None
    _products_cache_ts = 0.0


def _clone_last_result() -> Optional[Dict[str, Any]]:
    return copy.deepcopy(last_embedding_result) if last_embedding_result else None


def _record_perf_event(metric: str, duration_ms: float, **extra: Any) -> None:
    entry = {"ts": time.time(), "metric": metric, "duration_ms": float(duration_ms)}
    for key, value in extra.items():
        if value is not None:
            entry[key] = value
    face_perf_window.append(entry)


def _compute_iou(box_a: Optional[Tuple[int, int, int, int]], box_b: Optional[Tuple[int, int, int, int]]) -> float:
...
def _create_index() -> hnswlib.Index:
    new_index = hnswlib.Index(space='cosine', dim=DIMENSION)
    new_index.init_index(max_elements=MAX_ELEMENTS, ef_construction=EF_CONSTRUCTION, M=M_INDEX)
    return new_index
...
def init_hnsw_index():
    ...
    if INDEX_PATH.exists() and INDEX_LABELS_PATH.exists():
        try:
            existing_index = hnswlib.Index(space='cosine', dim=DIMENSION)
            existing_index.load_index(str(INDEX_PATH), max_elements=MAX_ELEMENTS)
            index = existing_index

            with open(INDEX_LABELS_PATH, 'r') as f:
                index_labels = json.load(f)

            print(f"✅ HNSW index loaded (dim={DIMENSION}, items={index.get_current_count()})")
            return
        except RuntimeError as exc:
            print(f"⚠️  Failed to load existing index: {exc}. Rebuilding...")
...
async def startup_event():
    ...
    ensure_column("transactions", "payment_method", "TEXT DEFAULT 'balance'")
    ensure_column("transactions", "point_of_sale_id", "INTEGER")
    ensure_column("transactions", "cashier_id", "INTEGER")
    ensure_column("transactions", "quantity", "INTEGER NOT NULL DEFAULT 1")
...
async def get_user(user_id: int, current_user: User = Depends(require_roles(Role.ADMIN))):
    with Session(engine) as session:
...
async def reset_user_password(user_id: int, payload: UserPasswordReset, current_user: User = Depends(require_roles(Role.ADMIN))):
    with Session(engine) as session:
        ...
        asyncio.create_task(email_service.send_password_reset(
            to=user.email,
            first_name=user.first_name or user.full_name or user.email,
...
async def list_products():
    ...
    if _products_cache is not None and (now - _products_cache_ts) < _PRODUCTS_CACHE_TTL_S:
        _log_stage("products.cache_hit", now)
    ...
    with Session(engine) as session:
        ...
        _products_cache_ts = time.perf_counter()
        _log_stage("products.query", start, count=len(products))
        return result


@app.get("/api/health/timing")
async def health_timing_summary():
    window_seconds = PERF_WINDOW_SECONDS
    recent_detect = [entry for entry in face_perf_window if entry.get("metric") == "detect" and (time.time() - entry.get("ts", 0)) <= window_seconds]
    recent_embed = [entry for entry in face_perf_window if entry.get("metric") == "embed" and (time.time() - entry.get("ts", 0)) <= window_seconds]
    request_entries = [entry for entry in request_metrics if (time.time() - entry.get("ts", 0)) <= window_seconds]

    return {
        "window_seconds": window_seconds,
        "face": {
            "detect": _calc_latency_stats(recent_detect),
            "embed": _calc_latency_stats(recent_embed),
        },
        "requests": _calc_latency_stats(request_entries),
        "last_detect": recent_detect[-1] if recent_detect else None,
        "last_embed": recent_embed[-1] if recent_embed else None,
    }
...
async def approve_topup(
    ...
    with Session(engine) as session:
        ...
        parent = session.get(User, topup.parent_id)
        if parent:
            asyncio.create_task(email_service.send_topup_approved(
                to=parent.email,
                parent_name=parent.first_name or parent.full_name or parent.email,
...
async def reject_topup(
    ...
    with Session(engine) as session:
        ...
        parent = session.get(User, topup.parent_id)
        if parent:
            asyncio.create_task(email_service.send_topup_rejected(
                to=parent.email,
                parent_name=parent.first_name or parent.full_name or parent.email,
                total_amount=topup.total_amount,
                currency=CURRENCY,
...
async def list_student_scheduled_orders(
    ...
    with Session(engine) as session:
        ...
        orders_query_ms = (time.perf_counter() - query_start) * 1000

        order_ids = [order.id for order in orders]
        items_query_start = time.perf_counter()
        items = (
            session.exec(select(ScheduledOrderItem).where(ScheduledOrderItem.order_id.in_(order_ids))).all()
            if order_ids
            else []
        ...
        items_query_ms = (time.perf_counter() - items_query_start) * 1000
        ...
        response = [serialize_scheduled_order(order, items_by_order.get(order.id, []), products) for order in orders]
        serialization_ms = (time.perf_counter() - serialization_start) * 1000
        total_ms = (time.perf_counter() - total_start) * 1000
        perf_logger.info(
            "scheduled_orders student=%s count=%s query_ms=%.2f items_ms=%.2f serialize_ms=%.2f total_ms=%.2f",
            student_id,
            len(response),
            orders_query_ms,
            items_query_ms,
...
async def parent_register(payload: ParentRegisterRequest):
    with Session(engine) as session:
        ...
        asyncio.create_task(email_service.send_welcome_parent(
            to=user.email,
            first_name=user.first_name or user.full_name or user.email,
        ))
...
async def approve_link_request(
    ...
    with Session(engine) as session:
        ...
        parent = session.get(User, request.parent_id)
        if parent:
            asyncio.create_task(email_service.send_link_request_approved(
                to=parent.email,
                parent_name=parent.first_name or parent.full_name or parent.email,
...
async def reject_link_request(
    ...
    with Session(engine) as session:
        ...
        parent = session.get(User, request.parent_id)
        if parent:
            asyncio.create_task(email_service.send_link_request_rejected(
                to=parent.email,
                parent_name=parent.first_name or parent.full_name or parent.email,
                student_name=request.student_name,
...
async def get_student_detail(student_id: str):
    ...
    _log_stage("students.detail", start, student_id=student_id)
    return payload
...
async def search_students(query: str = Query("", description="Search query for student name"), limit: int = Query(SEARCH_LIMIT, description="Maximum results")):
    ...
    with Session(engine) as session:
        ...
        _log_stage("students.search", start, query=query, count=len(students))
...
async def enroll_burst(
    ...
    burst_id: Optional[str] = Form(None),
):
    """Enroll student with multiple frames (3-5)"""
    _burst_tag = burst_id or "no-burst-id"
    logger.info("[enroll_burst] START burst_id=%s name=%s grade=%s frames=%d",
                _burst_tag, name, grade, len(frames))

    if len(frames) < 3 or len(frames) > 5:
        raise HTTPException(status_code=400, detail="Provide 3-5 frames")

    # --- Server-side dedup: reject if same name+grade was enrolled recently ---
    dedup_key = f"{name.strip().lower()}|{grade.strip().lower()}"
    now = time.time()
    cached = _burst_dedup_cache.get(dedup_key)
    if cached and (now - cached["ts"]) < BURST_DEDUP_WINDOW_S:
        logger.warning(
            "[enroll_burst] DEDUP HIT burst_id=%s dedup_key=%s previous_student_id=%s age=%.1fs",
            _burst_tag, dedup_key, cached["student_id"], now - cached["ts"],
        )
        return {
    ...
    try:
        ...
        for fi, frame in enumerate(frames):
            ...
            img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
            if img is None:
                logger.debug("[enroll_burst] burst_id=%s frame_%d: decode failed", _burst_tag, fi)
                continue
            ...
            if bbox is not None:
                ...
                if face is not None:
                    ...
                    if embedding is not None:
                        ...
                        logger.debug("[enroll_burst] burst_id=%s frame_%d: embedding OK", _burst_tag, fi)
            else:
                logger.debug("[enroll_burst] burst_id=%s frame_%d: no face detected", _burst_tag, fi)

        logger.info("[enroll_burst] burst_id=%s embeddings_collected=%d/%d",
                    _burst_tag, len(embeddings), len(frames))
        ...
        result_photo = f"data/faces/{photo_filename}" if best_frame is not None else ""

        # Store in dedup cache
        _burst_dedup_cache[dedup_key] = {
            "student_id": student_id,
            "photo_path": result_photo,
            "ts": time.time(),
        }
        # Evict stale entries
        stale_keys = [k for k, v in _burst_dedup_cache.items() if (now - v["ts"]) > BURST_DEDUP_WINDOW_S * 2]
        for k in stale_keys:
            del _burst_dedup_cache[k]

        logger.info("[enroll_burst] DONE burst_id=%s -> student_id=%s name=%s frames_used=%d",
                    _burst_tag, student_id, name, len(embeddings))
        ...
        logger.exception("[enroll_burst] burst_id=%s ERROR: %s", _burst_tag, e)
...
async def recognize_face(file: UploadFile = File(...)):
    ...
    global last_recognition_ts, current_no_face_backoff_ms
    global last_embedding_ts_ms, last_embedding_result, last_bbox, last_embedding_vector

    now = time.time()
    now_ms = int(now * 1000)
    elapsed = now_ms - int(last_recognition_ts * 1000)
    detection_stub = {"face_detected": False, "bbox": None, "confidence": 0.0}

    if elapsed < RECOGNITION_MIN_INTERVAL_MS:
        wait_ms = RECOGNITION_MIN_INTERVAL_MS - elapsed
        return {
            "match": False,
            "score": 0.0,
            "student": None,
            "detection": detection_stub,
            "throttle": {
                "reason": "min_interval",
                "retry_after_ms": wait_ms,
                "backoff_ms": current_no_face_backoff_ms,
            },
        }
    ...
    try:
        ...
        bbox = face_engine.detect_face_bgr(img)
        detect_duration_ms = (time.perf_counter() - detect_start) * 1000
        _record_perf_event("detect", detect_duration_ms, width=img_width, height=img_height)
        ...
        def respond(match: bool, score: float, student: Optional[Dict[str, Any]], reason: str, retry_ms: int) -> Dict[str, Any]:
            return {
                "match": match,
                "score": float(score),
                "student": student,
                "detection": copy.deepcopy(detection_data),
                "throttle": {
                    "reason": reason,
        ...
        def respond_cached(reason: str, retry_ms: int) -> Dict[str, Any]:
            cached = _clone_last_result()
            if cached:
                cached["detection"] = copy.deepcopy(detection_data)
                cached.setdefault("throttle", {})
                cached["throttle"].update(
                    reason=reason,
                    retry_after_ms=max(0, retry_ms),
                    backoff_ms=current_no_face_backoff_ms,
                )
                return cached
            return respond(False, 0.0, None, reason, retry_ms)

        if bbox is None:
            current_no_face_backoff_ms = min(
                MAX_NO_FACE_BACKOFF_MS,
                current_no_face_backoff_ms + NO_FACE_BACKOFF_MS,
            )
            return respond(False, 0.0, None, "no_face", current_no_face_backoff_ms)
        ...
        detection_data["confidence"] = 0.85

        cache_age_ms = now_ms - last_embedding_ts_ms if last_embedding_ts_ms else None

        if (
            EMBED_COOLDOWN_MS > 0
            and last_embedding_result
            and cache_age_ms is not None
            and cache_age_ms < EMBED_COOLDOWN_MS
        ):
            remaining = EMBED_COOLDOWN_MS - cache_age_ms
            return respond_cached("embed_cooldown", remaining)

        if (
            last_embedding_result
            and last_bbox
            and cache_age_ms is not None
            and FACE_CACHE_WINDOW_MS > 0
            and cache_age_ms <= FACE_CACHE_WINDOW_MS
            and _compute_iou(bbox, last_bbox) >= FACE_CACHE_IOU_THRESHOLD
        ):
            cached_resp = respond_cached("bbox_cache", RECOGNITION_MIN_INTERVAL_MS)
            last_bbox = bbox
            last_embedding_result = copy.deepcopy(cached_resp)
            return cached_resp

        face = face_engine.crop_align(img, bbox)
        if face is None:
            return respond(False, 0.0, None, "crop_failed", RECOGNITION_MIN_INTERVAL_MS)

        embed_start = time.perf_counter()
        embedding = face_engine.embed(face)
        embed_duration_ms = (time.perf_counter() - embed_start) * 1000
        _record_perf_event("embed", embed_duration_ms, width=face.shape[1], height=face.shape[0])

        if embedding is None:
            return respond(False, 0.0, None, "embed_unavailable", RECOGNITION_MIN_INTERVAL_MS)

        embedding = np.asarray(embedding, dtype=np.float32)
        last_embedding_ts_ms = now_ms
        last_bbox = bbox
        last_embedding_vector = embedding.copy()

        if index is None or index.get_current_count() == 0:
            response = respond(False, 0.0, None, "index_empty", RECOGNITION_MIN_INTERVAL_MS)
            last_embedding_result = copy.deepcopy(response)
            return response

        labels, similarities = search_similar(embedding, k=1)
        if not labels:
            response = respond(False, 0.0, None, "no_match", RECOGNITION_MIN_INTERVAL_MS)
            last_embedding_result = copy.deepcopy(response)
            return response
        ...
        score = float(similarities[0]) if similarities else 0.0

        if not student:
            response = respond(False, score, None, "student_missing", RECOGNITION_MIN_INTERVAL_MS)
            last_embedding_result = copy.deepcopy(response)
            return response
        ...
        response = respond(True, score, student_response, "face_detected", RECOGNITION_MIN_INTERVAL_MS)
        last_embedding_result = copy.deepcopy(response)
        return response
...
@app.get("/debug/perf")
async def debug_perf(current_user: User = Depends(require_roles(Role.ADMIN))):
    cutoff_ts = time.time() - PERF_WINDOW_SECONDS
    recent_events = [evt for evt in face_perf_window if evt["ts"] >= cutoff_ts]
    embed_events = [evt for evt in recent_events if evt["metric"] == "embed"]
    detect_events = [evt for evt in recent_events if evt["metric"] == "detect"]

    def avg_duration(events: List[Dict[str, Any]]) -> float:
        return float(sum(evt["duration_ms"] for evt in events) / len(events)) if events else 0.0

    last_age_ms = (time.time() * 1000 - last_embedding_ts_ms) if last_embedding_ts_ms else None

    return {
        "window_seconds": PERF_WINDOW_SECONDS,
        "events_tracked": len(recent_events),
        "embed_count": len(embed_events),
        "detect_count": len(detect_events),
        "avg_embed_ms": round(avg_duration(embed_events), 3) if embed_events else 0.0,
        "avg_detect_ms": round(avg_duration(detect_events), 3) if detect_events else 0.0,
        "cooldown_ms": EMBED_COOLDOWN_MS,
        "cache_window_ms": FACE_CACHE_WINDOW_MS,
        "cache_iou": FACE_CACHE_IOU_THRESHOLD,
        "detect_width_hint": FACE_DETECT_WIDTH,
        "last_embedding_age_ms": last_age_ms,
        "last_bbox": last_bbox,
        "recent_samples": recent_events[-5:],
    }
...
async def charge_student(
    ...
    with Session(engine) as session:
        ...
        stock_elapsed = (time.perf_counter() - stock_start) * 1000
        ...
        if charge.payment_method == "balance" and student.balance <= LOW_BALANCE_THRESHOLD:
            guardians = session.exec(
                select(StudentGuardian).where(StudentGuardian.student_id == student.id)
            ).all()
            for guardian in guardians:
                parent = session.get(User, guardian.parent_user_id)
                if parent:
                    asyncio.create_task(email_service.send_low_balance_alert(
                        to=parent.email,
                        parent_name=parent.first_name or parent.full_name or parent.email,
...
UPDATE_STATUS_FILE = Path(__file__).parent / "deploy" / ".current_version"
UPDATE_SCRIPT = Path(__file__).parent / "deploy" / "check_update.sh"
...
async def trigger_update(request: Request):
    """Trigger remote update of the caja machine"""
...
async def get_update_status(request: Request):
    ...
    if UPDATE_STATUS_FILE.exists():
        try:
            current_version = UPDATE_STATUS_FILE.read_text().strip()
        except Exception:
            pass
    ...
    if UPDATE_STATUS_FILE.exists():
        try:
            mtime = UPDATE_STATUS_FILE.stat().st_mtime
            last_check = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            pass
...
async def sync_students_to_backend(request: Request):
    """Push student list to backend for link request approvals"""
    verify_internal_token(request)
    
    backend_url = os.getenv("BACKEND_SYNC_URL")
    if not backend_url:
        raise HTTPException(status_code=500, detail="BACKEND_SYNC_URL not configured")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{backend_url}/api/sync.php?action=students",
        ...
        return {
            "success": True,
            "message": f"Synced {len(student_data)} students to backend",
            "count": len(student_data)
        }
    except Exception as e:
        logger.error(f"Failed to sync students to backend: {e}")
