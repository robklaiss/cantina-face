...
let _burstEnrolledId = null;  // tracks student created by burst to prevent duplicates
...
function setupEnrollEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        const isTypingField = (
            (target?.tagName === 'INPUT' || target?.tagName === 'TEXTAREA') && !target.readOnly && !target.disabled
        ) || target?.isContentEditable;

        // Space for capture
        if (!isTypingField && e.key === ' ' && !e.ctrlKey && !e.metaKey) {
            e.preventDefault();
        ...
        if (!isTypingField && e.key === ' ' && (e.shiftKey)) {
            e.preventDefault();
            capturePhoto('burst');
            return;
        }
...
async function captureBurstFrames() {
    ...
    // Generate a unique burst_id for traceability and server-side dedup
    const burstId = `burst_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    console.log(`[burst] starting burstId=${burstId} frames=${frames.length} attach=${isAttachMode}`);

    // Create FormData with multiple frames
    const formData = new FormData();
    formData.append('burst_id', burstId);
    ...
    if (isAttachMode) {
        ...
        if (result.photo_path) {
            ...
            embeddingState.textContent = '✅ Calculado';
            showNotification(`✅ Registrado: ${result.name} (ráfaga)`, 'success');
        }

        // Burst already created the student server-side.
        // Mark as enrolled and reset form to prevent duplicate via submitEnrollment.
        _burstEnrolledId = result.id || null;
        console.log(`[burst] enrolled student_id=${_burstEnrolledId}, name=${result.name}, frames=${result.frames_processed}`);
        resetForm();
    }
}
...
async function submitEnrollment() {
    ...
    // Guard: if burst already created this student, don't create again
    if (_burstEnrolledId) {
        console.warn(`[submitEnrollment] blocked: student already created by burst (id=${_burstEnrolledId})`);
        showNotification('El alumno ya fue registrado por la ráfaga', 'info');
        _burstEnrolledId = null;
        resetForm();
        return;
    }
