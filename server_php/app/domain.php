...
function list_student_link_counts(array $studentIds): array
{
    $studentIds = array_values(array_filter(array_map(static function ($studentId) {
        return trim((string) $studentId);
    }, $studentIds), static function ($studentId) {
        return $studentId !== '';
    }));
    ...
    $counts = [];
    foreach ($stmt->fetchAll() as $row) {
        $counts[(string) $row['student_id']] = (int) $row['total'];
    }

    return $counts;
}

function append_student_link_summaries(array $students): array
{
    if (!$students) {
        return [];
    }

    $counts = list_student_link_counts(array_column($students, 'id'));

    return array_map(static function (array $student) use ($counts): array {
        $studentId = (string) ($student['id'] ?? '');
        $linkedParentCount = (int) ($counts[$studentId] ?? 0);
        $student['is_linked'] = $linkedParentCount > 0;
        $student['linked_parent_count'] = $linkedParentCount;
        return $student;
    }, $students);
}

function find_student_record_by_id(string $studentId): ?array
{
...
function list_student_parent_links(string $studentId): array
{
    $studentId = trim($studentId);
    if ($studentId === '') {
        return [];
    ...
    $stmt = db()->prepare(
        'SELECT
         ...
         ORDER BY ps.created_at DESC, COALESCE(u.full_name, u.email) ASC'
    );
    ...
    return array_map(static function (array $row): array {
        ...
        if ($displayName === '') {
            $displayName = (string) ($row['email'] ?? ('Parent #' . ($row['parent_id'] ?? '')));
        }

        return [
            'parent_id' => isset($row['parent_id']) ? (int) $row['parent_id'] : null,
            'parent_name' => $displayName,
            'parent_email' => $row['email'] ?? null,
            'parent_phone' => $row['phone'] ?? null,
            'parent_dni' => $row['dni'] ?? null,
            'linked_at' => $row['linked_at'] ?? null,
        ];
    }, $stmt->fetchAll());
}

function update_link_request_status(int $requestId, string $status, ?string $studentId, ?string $adminNotes): void
{
    $pdo = db();
    try {
...
function update_student(int $studentId, array $data): void
{
    ...
    $sql = 'UPDATE students SET ' . implode(', ', $fields) . ', updated_at = :updated_at WHERE id = :id';
